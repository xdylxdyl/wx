...
public class ListItem implements ItemMethod {
        ...
        if (StringUtils.isNotEmpty(objectName)) {
            ...
            if (StringUtils.isNotEmpty(keyProperty)) {
                ...
                try {
                    for (int i = 0; i < keys.length; i++) {
                        ...
                        this.keyMethod[i] = MethodUtils.getAccessibleMethod(Class.forName(objectName), "get"
                                + WordUtils.capitalize(key), new Class[] {});
                    }
            ...
            if (StringUtils.isNotEmpty(valueProperty)) {
                try {
                    ...
                    valueMethod = MethodUtils.getAccessibleMethod(Class.forName(objectName), "get"
                            + WordUtils.capitalize(value), new Class[] {});
            ...
            if (StringUtils.isNotEmpty(orderByProperty)) {
                ...
                try {
                    for (int i = 0; i < keys.length; i++) {
                        String key = StringUtils.trim(keys[i]);
                        this.orderByMethod[i] = MethodUtils.getAccessibleMethod(Class.forName(objectName), "get"
                                + WordUtils.capitalize(key), new Class[] {});
                    }
                } catch (ClassNotFoundException e) {
                    if (logger.isDebugEnabled()) {
    ...
    public String getSqlcountitem() {
        if (StringUtils.isBlank(sqlcountitem) && StringUtils.isNotEmpty(this.sqlitem)) {
            // Fix 大小写的SQL
    ...
    public String getSqldelitem() {
        if (StringUtils.isBlank(sqldelitem) && StringUtils.isNotEmpty(this.sqlitem)) {
            // Fix 大小写的SQL
    ...
    public String getSqlAggregateItem() {
        if (StringUtils.isBlank(sqlAggregateItem) && StringUtils.isNotEmpty(this.sqlitem)) {
            // Fix 大小写的SQL
            this.sqlAggregateItem = SqlStringHelper.getAggregateSql(this.sqlitem);
        }
        return sqlAggregateItem;
    }
    ...
    public String toString() {
        ...
        buffer.append(" invalidOnSave = ").append(invalidOnSave);
    ...
    public String getReverseSql() {
        if (StringUtils.isBlank(reverseSql) && StringUtils.isNotEmpty(this.sqlitem)) {
