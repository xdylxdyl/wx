 ...
 * @author allenshen
 *         <p/>
 *         date: Nov 12, 2008 9:53:42 AM
 */
public interface Dao {
    /**
     * 保存一个持久化对象
     * 当保存一个持久化对象时，需要指定一个account_id（用户账号ID），这个account_id可以帮助我们把同一个用户的数据保存在一起（同一机器，同一数据库，等）。
     * 在使用是注意，在实现改接口是注意检查数据的有效性，比如保存的对象是不是指定account_id的对象。
     *
     * @param account_id ：用户Account_id,
     *                   在DAL中用他来作为策略的依据参数，如果是NULL策略会默认处理，比如数据字典的数据，不存在account_id,策略就会选择默认的方式处理
     * @param object     ：持久化对象
     * @return Serializable： 返回保存的持久化对象的ID
     * @throws Exception
     */
    public Serializable save(Object account_id, Object object)
            throws DaoException;
     ...
     *
     * @param account_id ：用户Account_id 如果account_id是null就会保存或者更新默认的库和CACHE
     * @param objects    ：要保存的持久化对象列表
     ...
     * 需要指定一个account_id（用户账号ID），这个account_id可以帮助我们把同一个用户的数据保存在一起（同一机器，同一数据库，等）。如果输入是null就会保存或者更新默认的库和CACHE
     ...
     *
     * @param account_id：用户Account_id
     * @param clazz                   ：要重载的持久化对象类别，
     * @param id                      ：持久化对象的标识
     * @return Object ：如果存在返回持久化对象。否则发回NLL
     * @throws DaoException
     */
    public Object get(Object account_id, Class clazz, Serializable id)
            throws DaoException;
     ...
     *
     * @param account_id ：用户Account_id
     * @param clazz      ：要装载的持久化对象。
     * @param ids        ：要装载的持久化对象标识列表
     * @return List ：返回装载的持久化对象，对象的类型是参数clazz指定的Class
     * @throws DaoException
     */

    public List getList(Object account_id, Class clazz, List ids)
            throws DaoException;
     ...
     *
     * @param objects ：要更新的持久化对象列表
     * @return List 持久化对象列表
     * @throws DaoException
     */

    public boolean batchUpdate(Object account_id, List objects)
            throws DaoException;
     ...
     * @param clazz      :
     *                   要删除的持久化对象的类型
     * @param id         ：要删除的持久化对象的标识
     * @return boolean ： 如果删除持久化对象成功返回true，否则false
     * @throws DaoException
     */
    public boolean delete(Object account_id, Class clazz, Serializable id)
            throws DaoException;
     ...
     * @throws DaoException
     */
    public boolean fakeDelete(Object account_id, Class clazz, Serializable id)
            throws DaoException;
     ...
     * @param clazz      :
     *                   要删除的持久化对象的类型
     * @param ids        ：要删除的持久化对象的标识列表
     * @return boolean： 如果删除成功返回true，否则返回false
     * @throws DaoException
     */
    public boolean deleteList(Object account_id, Class clazz, List ids)
            throws DaoException;
     ...
     *
     * @param account_id ：用户Account_id，
     * @param list_name  ：待删除的List所对应的list_name
     * @param params     :
     ...
     * @param forward：装载的反向，是向前还是向后
     * @return
     * @throws DaoException
     */
    public List getIdList(Object account_id, String list_name, Object param,
     ...
     * @throws DaoException
     */
    public List getIdList(String list_sql_name, Object param, Integer start,
    ...
    public List getIdList(String list_name, Object param, boolean forward)
            throws DaoException;
     ...
     * @param list_name  ：要查询的列表名称
     * @param param      ：列表的标识ID
     * @return 列表的长度
     * @throws DaoException
     */
    public List getIdList(Object account_id, String list_name, Object[] params,
                          Integer start, Integer count, boolean forward) throws DaoException;
     ...
     * @throws DaoException
     */
    public List getIdList(String list_name, Object[] params, Integer start,
                          Integer count, boolean forward) throws DaoException;
     ...
     * @throws DaoException
     */
    public List getIdList(String list_name, Object[] params, boolean forward)
            throws DaoException;
     ...
     * @throws DaoException
     */
    public int count(Object account_id, String list_name, Object[] params)
            throws DaoException;

    public int count(String list_name, Object[] params) throws DaoException;

    /**
     * 通过一个持久化对象的属性值找一个持久化对象。需要指定account_id(用户ID)以方便策略选择从哪个库或者CACHE中查询数据，如果输入是null就从默认的库或者CACHE中查询
     *
     * @param account_id  ：用户Account_id
     * @param mappingName ：持久化对象属性的名称标识
     * @param key         ：对象的属性值
     * @return ：持久化对象
     * @throws DaoException
     */
    public Object getMapping(Object account_id, String mappingName, Object[] keys)
            throws DaoException;

    public Object getMapping(String mappingName, Object[] keys)
            throws DaoException;

    public Object getMapping(Object account_id, String mappingName, Object key)
            throws DaoException;
